...
maps = [
    ("SKP", "karte_skp.png", BLUE,
     "Öffentlich belegt nur Kreis Pinneberg. Blass: Angabe des Unternehmens."),
    ("AWO Schleswig-Holstein", "karte_awo.png", AMBER,
     "Neun Kreise und kreisfreie Städte, eigene Bereichsleitungen je Region."),
    ("Lebenshilfe", "karte_lebenshilfe.png", OLIVE,
     "Vier Kreise mit belegter Schulbegleitung, Mitgliedsverbände fast landesweit."),
]
my = 1.86
for name, fn, c, note in maps:
    ...
    body(s, 10.72, my + 0.32, 1.87, 0.70, note, size=7.6, color=MUT)
    my += 1.40
...
card_bullets(s, COL[0], 3.66, CW, 2.42, C_BLUE, "Nachher", [
...
card_bullets(s, COL[1], 3.66, CW, 2.42, C_BLUE, "Nachher", [
...
card_bullets(s, COL[2], 1.92, CW, 1.62, C_GRAY, "Dafür", [
    ...
    "Kommunale Kostenträger · Robert Bosch und Telekom Stiftung · DISW-Evaluation",
], head_color=OLIVE, marker=OLIVE)
card_bullets(s, COL[2], 3.66, CW, 1.42, C_AMBER, "Dagegen", [
    "ver.di (24.04.2026): Fachkräftegebot werde durch einen Kompetenzansatz ersetzt",
    "Lebenshilfe Bundesverband (27.04.2026) · AGJ · SoVD · Verfassungsblog: nicht "
    "verfassungskonform",
], head_color=AMBER, marker=AMBER)
card_text(s, COL[2], 5.24, CW, 0.84, C_BLUE, None, [
    ("Entschieden ist es trotzdem: Das OVG Schleswig wies die Klage von vier Trägern "
     "am 03.09.2024 unanfechtbar zurück (5 MB 7/24).",
     {"size": 8.2, "bold": True, "color": TXT}),
])

source(s, "Quellen: Kreis Pinneberg, Konzept „Klassenassistenz“, Kap. 4–6 · TED 246925-2024 · "
          "Landtag SH, Presseticker 28.02.2025 und Drs. 20/3271 · ver.di, „Exklusion statt "
          "Inklusion“, 24.04.2026 · Bundesvereinigung Lebenshilfe, 27.04.2026 · OVG Schleswig "
          "5 MB 7/24 · Dworschak u. a. zur Qualifikationsstruktur.")
notes(s, "Drei Spalten, drei Verschiebungen. Erstens Qualifikation: Unten bleibt es ungelernt, "
         ...
         "neun Jahre ohne Tarifausgleich. Das ist der ökonomische Kern. Drittens Machtverhältnis: "
         "Es gibt laute Kritik, aber sie hat keine Hebelwirkung — der Kreis zahlt, der Kreis "
         "vergibt, und der Rechtsweg ist zu.")
...
questions = [
    ...
    ("2 · Löhne und Tarif", C_BLUE, BLUE,
     ...
     "Messlatte: Pinneberg kalkuliert mit TVöD SuE S2 Stufe 5 — rund 70 % über dem "
...
cd.categories = ["Mindest-\nlohn 2026", "Mindest-\nlohn 2027",
                 "TVöD SuE\nS2/5", "TVöD SuE\nS3/5"]
cd.add_series("Bruttolohn je Stunde", (13.90, 14.60, 23.57, 26.57))
gf = s.shapes.add_chart(XL_CHART_TYPE.COLUMN_CLUSTERED, Inches(1.46), Inches(1.90),
                        Inches(3.75), Inches(2.45), cd)
ch = gf.chart
style_chart(ch)
ch.value_axis.maximum_scale = 30.0
ch.value_axis.major_unit = 10.0
...
body(s, COL[0], 4.42, CW, 0.44,
     "Der Tarifsatz, mit dem Pinneberg kalkuliert, liegt 70 % über dem Mindestlohn. "
     "S2 Stufe 5 = 3.997,10 € bei 39 Wochenstunden.", size=7.8, color=MUT)

colhead(s, COL[1], TOP, "Nachfrage — Schulbegleitungen in SH")
cd = CategoryChartData()
cd.categories = ["2014", "2022"]
cd.add_series("Schulbegleitungen", (2700, 7000))
...
fill_series(ch.plots[0].series[0], BLUE)
ch.plots[0].gap_width = 110
data_labels(ch.plots[0], size=8.0, fmt="#,##0")
body(s, COL[1], 4.42, CW, 0.44,
     "+159 % in acht Jahren — und das, obwohl die Schulische Assistenz 2015 gerade "
     "eingeführt wurde, um die Zahl zu senken.", size=7.8, color=MUT)

colhead(s, COL[2], TOP, "Vergütung und Personalschlüssel")
card_text(s, COL[2], 1.90, CW, 1.25, C_BLUE, "So rechnet der Kreis", [
    ("Festbetrag aus TVöD SuE S2 Stufe 5, plus 10 % Sachkosten, plus 5 % "
     "Verwaltungskostenzuschlag auf die Personalkosten.", {"size": 8.4}),
])
card_text(s, COL[2], 3.28, CW, 1.07, C_GRAY, "Personalschlüssel", [
    ("Eine Klassenassistenz je Klasse — 24 Wochenstunden in Klasse 1–2, 30 in "
     "Klasse 3–4. Teamleitung 1:15.", {"size": 8.4}),
])
...
source(s, "Quellen: Mindestlohnkommission/BMAS · TVöD SuE, Tabelle gültig 01.05.2026–31.03.2027 "
          "(VKA, 39 Wochenstunden) · Kreis Pinneberg, Konzept „Klassenassistenz“, Kap. 5–6 · "
          "Landtag SH, Rede der Ministerin 28.02.2025 (2.700 → rund 7.000 Schulbegleitungen) · "
          "TED 246925-2024 · OVG Schleswig 5 MB 7/24. Stundensätze eigene Umrechnung.")
notes(s, "Vier Zahlen zum Merken. Erstens: Der Tarifsatz, mit dem der Kreis kalkuliert, liegt "
         "70 Prozent über dem Mindestlohn — wer heute Mindestlohn zahlt, hat im Pool zunächst "
         "Luft. Zweitens: Die Nachfrage hat sich in acht Jahren mehr als verzweieinhalbfacht. "
         "Drittens: Das Budget hängt an Klassen, nicht an Fällen — die Menge ist damit planbar. "
         "Viertens, und das ist der Haken: Der Preis steht bis zu neun Jahre fest, "
         "Tarifsteigerungen trägt der Träger allein. Deshalb ist die Kalkulation beim Angebot "
         "die wichtigste einzelne Entscheidung.")
